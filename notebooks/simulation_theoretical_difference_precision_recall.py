...
def compute_auprc(data):
    """
    Compute AUPRC (Area Under Precision-Recall Curve) on pooled data and as weighted average across folds.
    ...
    dict
        ...
        - 'auprc_pooled': AUPRC computed on all data pooled together
        - 'auprc_weighted': Weighted average of per-fold AUPRC (weighted by Nk/N)
        - 'auprc_per_fold': Array of AUPRC values for each fold
    """
    y = data["y"]
    ...
    # Convert scores: average_precision_score expects higher scores for positive class
    # Since y_score represents probability of class 0, we need to invert it
    y_score_inverted = 1 - y_score

    # Compute pooled AUPRC (entire vector)
    auprc_pooled = average_precision_score(y, y_score_inverted)

    # Compute AUPRC per fold
    auprc_per_fold = np.zeros(k)
    for fold in range(k):
        start_idx = fold * Nk
        end_idx = start_idx + Nk

        fold_y = y[start_idx:end_idx]
        fold_scores = y_score_inverted[start_idx:end_idx]

        # Check if fold has both classes (needed for AUPRC calculation)
        if len(np.unique(fold_y)) > 1:
            auprc_per_fold[fold] = average_precision_score(fold_y, fold_scores)
        else:
            auprc_per_fold[fold] = np.nan

    # Compute weighted average of per-fold AUPRC
    # Weight = Nk / N (each fold has equal weight if all folds have same size)
    weight = Nk / N
    auprc_weighted = np.nansum(auprc_per_fold * weight)

    return {
        "auprc_pooled": auprc_pooled,
        "auprc_weighted": auprc_weighted,
        "auprc_per_fold": auprc_per_fold,
    }
...
def run_simulation(Ks, N, positive_ratios, case="single_overlap"):
    ...
    auprc_results = np.zeros((len(Ks), len(positive_ratios)))
    ...
    for i, K in enumerate(Ks):
        for j, positive_ratio in enumerate(positive_ratios):
            ...
            auprc_metrics = compute_auprc(data)

            # difference between pooled and weighted
            auprc_diff = auprc_metrics["auprc_weighted"] - auprc_metrics["auprc_pooled"]

            # Store results
            auprc_results[i, j] = auprc_diff

    return auprc_results


def plot_simulation_results(axes, row, auprc_results, Ks, positive_ratios):
    """
    Plot simulated AUPRC results for different K values on a given axes row.
    ...
    auprc_results : np.ndarray
        AUPRC difference results matrix (shape: len(Ks) x len(positive_ratios)).
    Ks : list
        List of K values (number of folds).
    positive_ratios : np.ndarray
        Array of positive class ratios.
    """
    # Plot AUPRC differences
    for i, K in enumerate(Ks):
        color = f"C{i}"
        axes[row, 2].plot(positive_ratios, auprc_results[i], marker="o", markersize=4, label=f"K={K}", color=color)

    axes[row, 2].grid(True, alpha=0.3)
    axes[row, 2].set_ylabel(r"$AUPRC_{Average} - AUPRC_{Pooled}$")
...
if __name__ == "__main__":
    ...
    auprc_results = run_simulation(Ks, N, positive_ratios, case="single_overlap")
    plot_simulation_results(axes, 0, auprc_results, Ks, positive_ratios)
    ...
    auprc_results = run_simulation(Ks, N, positive_ratios, case="two_overlap")
    plot_simulation_results(axes, 1, auprc_results, Ks, positive_ratios)
    ...
    auprc_results = run_simulation(Ks, N, positive_ratios, case="three_overlap")
    plot_simulation_results(axes, 2, auprc_results, Ks, positive_ratios)
    ...
    axes[2, 2].set_xlabel("Positive Ratio")

    fig.tight_layout()
    fig.savefig("auprc_simulated.pdf")
    fig.savefig("auprc_simulated.png")
